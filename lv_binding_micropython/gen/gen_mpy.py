...
for enum_def in enum_defs:
    ...
    for member in enum_def.type.values.enumerators:
        ...
        if len(enum_name) > 0 and get_enum_name(enum_name) != "ENUM":
            enum[member_name] = "MP_ROM_INT(%s)" % member.name.replace("ENUM_", "")
        else:
            int_constants.append(member.name.replace("ENUM_", ""))
    if len(enum) > 0:
...
for enum in [
    ...
    int_constants.append("%s_%s" % (enum.replace("ENUM_", ""), next(iter(enums[enum])).replace("ENUM_", "")))
    del enums[enum]
