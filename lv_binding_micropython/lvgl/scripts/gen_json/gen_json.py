...
def run(output_path, lv_conf_file, output_to_stdout, target_header, filter_private, no_docstrings, *compiler_args):
    ...
    try:
        ...
        if sys.platform.startswith('win'):
            # Try to use GCC first (especially for ESP-IDF builds)
            cpp_cmd = ['riscv32-esp-elf-gcc', '-std=c11', '-E', '-Wno-incompatible-pointer-types']
            include_path_env_key = 'C_INCLUDE_PATH'
            output_pp = f' >> "{pp_file}"'
            
            # Check if riscv32-esp-elf-gcc exists
            try:
                subprocess.check_call(['riscv32-esp-elf-gcc', '--version'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except:
                ...
                try:
                    ...
                    # Final fallback to original MSVC logic if possible
                    import get_sdl2
                    try:
                        import pyMSVC  # NOQA
                        ...
                        sys.stderr.write('\nNo suitable C preprocessor found (gcc or MSVC).\n')
                        sys.exit(-500)
