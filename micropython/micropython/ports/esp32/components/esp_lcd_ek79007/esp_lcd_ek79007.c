...
static esp_err_t panel_ek79007_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                      bool mirror_y);
static esp_err_t panel_ek79007_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
static esp_err_t panel_ek79007_invert_color(esp_lcd_panel_t *panel,
                                            bool invert_color_data);

esp_err_t
esp_lcd_new_panel_ek79007(const esp_lcd_panel_io_handle_t io,
                          const esp_lcd_panel_dev_config_t *panel_dev_config,
                          esp_lcd_panel_handle_t *ret_panel) {
  ESP_LOGI(TAG, "version: %d.%d.%d", ESP_LCD_EK79007_VER_MAJOR,
           ESP_LCD_EK79007_VER_MINOR, ESP_LCD_EK79007_VER_PATCH);
  ESP_RETURN_ON_FALSE(io && panel_dev_config && ret_panel, ESP_ERR_INVALID_ARG,
                      TAG, "invalid arguments");
  ek79007_vendor_config_t *vendor_config =
      (ek79007_vendor_config_t *)panel_dev_config->vendor_config;
  ...
  ek79007_panel_t *ek79007 =
      (ek79007_panel_t *)calloc(1, sizeof(ek79007_panel_t));
  ...
  (*ret_panel)->swap_xy = panel_ek79007_swap_xy;
...
static const ek79007_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0x80, (uint8_t[]){0x8B}, 1, 0}, {0x81, (uint8_t[]){0x78}, 1, 0},
    {0x82, (uint8_t[]){0x84}, 1, 0}, {0x83, (uint8_t[]){0x88}, 1, 0},
    {0x84, (uint8_t[]){0xA8}, 1, 0}, {0x85, (uint8_t[]){0xE3}, 1, 0},
    {0x86, (uint8_t[]){0x88}, 1, 0}, {0x11, (uint8_t[]){0x00}, 0, 120},
};
...
static esp_err_t panel_ek79007_send_init_cmds(ek79007_panel_t *ek79007) {
  ...
  ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, EK79007_PAD_CONTROL,
                                                (uint8_t[]){
                                                    lane_command,
                                                },
                                                1),
  ...
  if (ek79007->init_cmds) {
    ...
    init_cmds_size =
        sizeof(vendor_specific_init_default) / sizeof(ek79007_lcd_init_cmd_t);
  }
...
static esp_err_t panel_ek79007_init(esp_lcd_panel_t *panel) {
  ...
  ESP_RETURN_ON_ERROR(panel_ek79007_send_init_cmds(ek79007), TAG,
...
static esp_err_t panel_ek79007_swap_xy(esp_lcd_panel_t *panel, bool swap_axes) {
  if (swap_axes) {
    ESP_LOGW(TAG, "Hardware swap_xy mapping is not supported natively by the "
                  "EK79007 driver.");
    return ESP_ERR_NOT_SUPPORTED;
  }
  // LVGL issues a swap_xy(false) call by default inside esp_lvgl_port_disp.c
  // Silently consume the "false" directive so the ESP runtime doesn't panic.
  return ESP_OK;
}
