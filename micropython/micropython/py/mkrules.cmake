...
# Provide defaults for preprocessor flags
message(STATUS "DEBUG: Building MICROPY_CPP_FLAGS VERSION 2 for ${MICROPY_TARGET}")
set(MICROPY_CPP_INC "")
set(MICROPY_CPP_DEF "")
...
# Ensure genhdr directory is in the include path for QSTR generation
list(APPEND MICROPY_CPP_INC "${MICROPY_GENHDR_DIR}" "${CMAKE_BINARY_DIR}")
...
foreach(_arg ${MICROPY_CPP_INC})
    ...
    if("${_arg_stripped}" STREQUAL "" OR "${_arg_stripped}" STREQUAL " ")
        continue()
    endif()
    list(APPEND MICROPY_CPP_FLAGS ${_prefix}${_arg_stripped})
endforeach()

set(_prefix "-D")
foreach(_arg ${MICROPY_CPP_DEF})
    string(STRIP "${_arg}" _arg_stripped)
    if("${_arg_stripped}" STREQUAL "")
        continue()
    endif()
    list(APPEND MICROPY_CPP_FLAGS ${_prefix}${_arg_stripped})
endforeach()
...
add_custom_command(
    ...
    set(MICROPY_CPP_FLAGS_QUOTED "")
    foreach(_flag ${MICROPY_CPP_FLAGS})
        string(STRIP "${_flag}" _flag_stripped)
        if(NOT "${_flag_stripped}" STREQUAL "")
            # Escape backslashes and quotes inside the flag
            string(REPLACE "\\" "\\\\" _flag_escaped "${_flag_stripped}")
            string(REPLACE "\"" "\\\"" _flag_escaped "${_flag_escaped}")
            # If it has spaces, quote it
            if(_flag_escaped MATCHES " ")
                list(APPEND MICROPY_CPP_FLAGS_QUOTED "\"${_flag_escaped}\"")
            else()
                list(APPEND MICROPY_CPP_FLAGS_QUOTED "${_flag_escaped}")
            endif()
        endif()
    endforeach()
    string(REPLACE ";" "\n" MICROPY_CPP_FLAGS_SPACED "${MICROPY_CPP_FLAGS_QUOTED}")
