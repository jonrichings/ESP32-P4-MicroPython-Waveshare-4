        ...
        def run(files):
            ...
            with os.fdopen(fd, 'w') as f:
                for arg in flags + files:
                    ...
                    if not arg_stripped or arg_stripped in ("-I", "-D"):
                        continue
                    # Quote arguments containing spaces or backslashes
                    if ' ' in arg_stripped or '\\' in arg_stripped:
                        arg_escaped = '"' + arg_stripped.replace('\\', '\\\\').replace('"', '\\"') + '"'
                    else:
                        arg_escaped = arg_stripped.replace('"', '\\"')
                    f.write(arg_escaped + '\n')
